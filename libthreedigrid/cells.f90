module m_cells
    ...
    subroutine set_2d_computational_nodes_lines(origin, lgrmin, kmax, mmax, nmax, dx, lg,&
        quad_idx, area_mask, nodk, nodm, nodn, bounds, coords, pixel_coords,&
            ...
            do m=1,mmax(k)
                do n=1,nmax(k)
                    ...
                    if(all(quad_idx(mn(1):mn(3),mn(2):mn(4)) == nod)) then
                        ...
                        call set_2d_computational_lines(l_u, l_v, k, m, n, mn, lg, lgrmin,&
                            area_mask_padded, quad_idx, nod, line, cross_pix_coords)
                        nod = nod + 1
                    else
                        continue
                    endif
