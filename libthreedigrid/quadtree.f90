module m_quadtree
    ...
    subroutine set_refinement(refine_id, refine_geom, refine_level,&
        refine_type, bbox, mmax, nmax, dx, lg)
        
        use m_grid_utils, only : get_cell_geom, find_cell_intersects,&
                                 geom_in_polygon,&
                                 feature_in_bbox

        double precision, intent(in) :: refine_geom(:, :) ! 2D array (x,y) of the refinement geometry
        integer, intent(in) :: refine_id ! 2D ID of the refinement geometry  
        integer, intent(in) :: refine_level ! The level of k to be set by the given geometry.
        integer, intent(in) :: refine_type ! Type of geometry passed in the refine_geom variable (LINESTRING or POLYGON)
        double precision, intent(in) :: bbox(4) ! Bounding box of total Quadtree
    ...
    !!! Entry point for creating quadtree by setting lg array and then finding number of active cells and lines.
        ...
        integer, intent(inout) :: n_lines(2) ! counter for active u lines
        integer :: m, n
        ...
        call find_active_2d_comp_cells(kmax, mmax, nmax, lgrmin, lg, area_mask, quad_idx, n_cells, n_lines(1), n_lines(2))
