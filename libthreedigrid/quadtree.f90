module m_quadtree
    ...
    subroutine make_quadtree(kmax, mmax, nmax, lgrmin, use_2d_flow, area_mask, lg, quad_idx,&
        ...
        use m_grid_utils, only : get_pix_corners, get_lg_corners, pad_area_mask

        integer, intent(in) :: kmax ! Maximum refinement levels
        integer, intent(in) :: mmax(:) ! Number of elements to fill X dimension per refinement level k
        integer, intent(in) :: nmax(:) ! Number of elements to fill Y dimension per refinement level k
        ...
        write (*,*) 'Initial i0, i1, j0, j1: ', i0, i1, j0, j1
        area_mask_padded = pad_area_mask(area_mask, i0, i1, j0, j1)
        write (*,*) 'area mask size: ', size(area_mask, 1), size(area_mask, 2)
        write (*,*) 'padded area mask size: ', size(area_mask_padded, 1), size(area_mask_padded, 2)
        do k=kmax,1,-1
            do m=1,mmax(k)
                do n=1,nmax(k)
    ...
    !!! Recursive subroutine to set correct refinement levels on lg refinement array.
        ...
        mn = get_lg_corners(k, m, n)
        if(any(lg(mn(1):mn(3),mn(2):mn(4)) < k)) then
            k1 = k-1
            m_n=2*m-1
            n_n=2*n-1
