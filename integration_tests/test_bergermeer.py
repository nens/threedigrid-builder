...
def test_integration(tmp_path):
    ...
    with h5py.File(tmp_path / "gridadmin.h5", "r") as f:
        ...
        ## NODES
        assert f["nodes"]["id"].shape == (13190,)  # Inpy: (13280, )
        assert_array_equal(f["nodes"]["id"][:], np.arange(f["nodes"]["id"].shape[0]))
        assert count_unique(f["nodes"]["node_type"]) == {
            -9999: 1,
            NodeType.NODE_2D_OPEN_WATER: 5329,
            NodeType.NODE_2D_GROUNDWATER: 5329,
        ...
        manholes_idx = np.where(f["nodes"]["is_manhole"][:] == 1)[0]
        assert np.all(f["nodes"]["zoom_category"][manholes_idx] != -9999)
        ...
        assert count_unique(f["lines"]["kcu"]) == {
            ...
            LineType.LINE_2D: 9464,  # Inpy: 9553
            LineType.LINE_2D_OBSTACLE: 1474,  # Inpy: 1484
            LineType.LINE_2D_VERTICAL: 5329,
        }
