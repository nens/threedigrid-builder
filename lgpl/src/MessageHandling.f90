...
!
!-------------------------------------------------------------------------------
!  $Id: MessageHandling.f90 4416 2014-11-20 12:33:00Z baart_f $
...
subroutine set_logger(c_callback) bind(C, name="set_logger")
  ...
  ! For debugging
  thresholdLvl_callback = LEVEL_DEBUG

  ! call SetMessagehandling(printToStdout, .true., mdia, thresholdLevel_file = LEVEL_DEBUG, thresholdLevel_log = LEVEL_DEBUG, thresholdLevel_stdout = LEVEL_INFO)
  call SetMessageHandling(.false., .true., 0, thresholdLevel_file = LEVEL_DEBUG, thresholdLevel_log = LEVEL_DEBUG, thresholdLevel_stdout = LEVEL_INFO)
  call mess(LEVEL_DEBUG, 'Log from Fortran.')
