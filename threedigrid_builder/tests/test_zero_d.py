...
def test_surfaces(grid_all):
    ...
    grid_all.surfaces = GridSurfaces(id=[])
    grid_all.nodes.content_type[1:] = ContentType.TYPE_V2_CONNECTION_NODES.value
    surface.apply(grid_all)

    surfaces = grid_all.surfaces
    ...
    expected = (
        ...
        ("cci", (3,), "int32"),
        ("cid", (3,), "int32"),
        ("surface_class", (0,), "object"),
        ("surface_inclination", (0,), "object"),
        ("surface_sub_class", (0,), "object"),
    )
    ...
    assert np.all(surfaces.centroid_x == np.array([1.0, 1.5]))
    assert np.all(surfaces.centroid_y == np.array([2.0, 2.5]))
    assert np.all(surfaces.cid == np.array([1, 2, 1]))
...
def test_impervioussurfaces(grid_all):
    ...
    assert np.all(
        surfaces.outflow_delay == np.array([0.5 / 60.0, 0.2 / 60.0], dtype="float32")
    )
