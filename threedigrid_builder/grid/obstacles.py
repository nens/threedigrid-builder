...
def apply_obstacles(lines, obstacles):
    ...
    idx = np.where(lines.kcu[inscts[1, :]] == LineType.LINE_2D_U)[0]
    lines.kcu[inscts[1, :]][idx] = LineType.LINE_2D_OBSTACLE_U
    idx = np.where(lines.kcu[inscts[1, :]] == LineType.LINE_2D_V)[0]
    lines.kcu[inscts[1, :]][idx] = LineType.LINE_2D_OBSTACLE_V
