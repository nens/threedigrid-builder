...
class ConnectedPoints(Array[ConnectedPoint]):
    ...
    def get_lines(
        ...
        if n_lines != line_node_idx.shape[0]:
            ...
            object_pk_msg = nodes.format_message(
                out_of_bounds[out_of_bounds_cp == -9999]
            )
            msg = ""
            if len(object_pk_msg) > 0:
                msg += (
    ...
    def get_breaches(self, lines, levees):
        ...
        line_idx = line_idx[np.argsort(lines.content_pk[line_idx])]
        breaches = PotentialBreaches(
            id=lines.content_pk[line_idx],
